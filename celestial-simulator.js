...
function makeGlow(colorInner, colorOuter, worldSize){
  ...
  /* 尾端用同色系透明而非透明黑,並改用加法混合:
     避免行動裝置 GPU 在半透明邊緣出現綠色雜點(透明黑內插導致的色偏) */
  const outer0=colorOuter.replace(/[\d.]+\)\s*$/,'0)');
  g.addColorStop(0,colorInner); g.addColorStop(0.35,colorOuter); g.addColorStop(1,outer0);
...
let invDrag=true, trackMode='east', lockMode='none'; /* 黃道軸置中預設開啟 */
...
const camL=new THREE.PerspectiveCamera(45,1,0.1,3000);
const CAM_REF=360;
...
const SCALE=30, POW=0.42, K_TRUE=3.2;
let trueScale=false;
...
const SPHERE_R=170;
/* 真實半徑(km):行星彼此比例嚴格正確(含月球),整體放大約 5,600 倍
   (完全真實下所有行星皆小於一個像素) */
const TRUE_KM=[2440,6052,6371,3390,69911,58232,25362,24622,1188];
const PLANET_EXAG=5.8e-5; /* 地球 0.37、月球 0.10(=地球/3.67)、木星 4.06 */
...
function applyScaleMode(){
  ...
  for(let i=0;i<ELEM.length;i++){
    const f=trueScale? (TRUE_KM[i]*PLANET_EXAG)/ELEM[i].size : 1; /* 無下限:比例嚴格 */
    planetMeshes[i].scale.setScalar(f);
    if(i===EARTH_IDX)earthF=f;
  }
  /* 太陽:與行星同倍率(半徑 ~40)——誠實呈現「尺寸放大 5,600 倍」套到
     太陽上的後果:巨殼會涵蓋到土星軌道。以半透明殼顯示,內部軌道仍可見 */
  sunMesh.scale.setScalar(trueScale? (696000*PLANET_EXAG)/5 : 1);
  sunMesh.material.transparent=true;
  sunMesh.material.opacity=trueScale? 0.13 : 1;
  sunMesh.material.depthWrite=!trueScale;
  sunGlow.visible=!trueScale;
  sunCore.visible=trueScale;
  moonF=trueScale? (1737.4*PLANET_EXAG)/0.55 : 1; /* 月球=地球/3.67,比例正確 */
  moonMesh.scale.setScalar(moonF);
  /* 月距:緊貼地球(真實月距僅 0.0026 AU,在行星尺寸誇大下取地球半徑外緣) */
  moonVisR=trueScale? 0.62 : 6.5;
  /* 影錐:半徑跟隨星體縮放、長度獨立縮放(確保真實比例下地影錐仍越過月軌) */
  const uLen=trueScale? 0.09 : 1;
  umbraCone.scale.set(earthF,uLen,earthF);
  umbraHalf=UMBRA_LEN*uLen/2;
  const mLen=trueScale? 0.25 : 1;
  mShadowCone.scale.set(moonF,mLen,moonF);
  mShadHalf=3.6*mLen/2;
  tidalGroup.scale.setScalar(trueScale? 0.35 : 1);
  buildOrbits();
}
...
earthSpin.add(earthGlobe);
{
  ...
  const dot=new THREE.Mesh(new THREE.SphereGeometry(0.28,10,8),new THREE.MeshBasicMaterial({color:0xff5a4d}));
  dot.position.set(1.55,0,0); earthSpin.add(dot);
}
...

{
  ...
  ground.rotation.x=-Math.PI/2; ground.renderOrder=1; sceneR.add(ground);
  const hor=[]; for(let k=0;k<=128;k++){const a=k/128*2*Math.PI;hor.push(new THREE.Vector3(Math.cos(a)*DOME,0,Math.sin(a)*DOME));}
  sceneR.add(new THREE.Line(new THREE.BufferGeometry().setFromPoints(hor),
    new THREE.LineBasicMaterial({color:0xE3B34C})));
  const horLbl=mkLbl('R',()=>T('地平線','Horizon'),'#E3B34C',3.4,false); horLbl.position.set(DOME*0.72,1.8,DOME*0.55); sceneR.add(horLbl);
  ...
  [30,60].forEach(alt=>{
    ...
    sceneR.add(new THREE.Line(new THREE.BufferGeometry().setFromPoints(pts),gmat));
  });
  ...
  dirs.forEach(d=>{
    ...
    l.position.set(d[1]*DOME*0.93,4,d[2]*DOME*0.93); sceneR.add(l);
  });
}
...
function applyLang(){
  ...
  [...ls.options].forEach((o,i)=>o.textContent=LOCK_STR[i][k]);
...
function animate(now){
  ...
  axisLine.geometry.setFromPoints([ew, axisW.clone().multiplyScalar(SPHERE_R*0.98)]);
  ...
  for(const L of labelsL){
    ...
    let f=Math.max(0.02,Math.min(8,sunWorld.distanceTo(camL.position)/CAM_REF));
  ...
  if(lockMode!=='none'){
    const tgt=lockMode==='sun'? sunWorld : moonWorld;
    if(tgt.lengthSq()>1e-6){
      const aim=tmpVR.copy(tgt).normalize();
      if(trackMode!=='off'){
